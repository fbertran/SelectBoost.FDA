...
`run_metadata.yml`, `RUNNING`, and `COMPLETED`; `RUNNING` is removed only
after successful completion. Use distinct `--output-dir` values for
parallel runs. `--resume` preserves previous checkpoint files but does
not yet skip completed settings. To check stability across sample size and
...
benchmark driver uses a recorded deterministic
